...
// ConnectDatabase 连接 SQL Server 数据库
//
// 参数:
...
//
// 说明:
//   - 使用 ? 作为统一占位符，最终会被转换为 SQL Server 的 @p1,@p2,... 格式
//   - 支持在事务中使用
...
func (engine *Engine) Migrate(schema string, model Model) {
	...
	row := engine.QueryRow("SELECT COUNT(1) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ?", schema, settings.TABLE_NAME)
...
//   - 使用 ? 作为统一占位符，最终会在 Execute/Query 中被转换为 SQL Server 的 @p1,@p2,... 格式
...
// 说明:
//   - 字段名会被自动去除首尾空格和引号
//   - 字段名最终会使用 SQL Server 的方括号引用: [column]
//   - 空字段名会被忽略
//   - 多次调用会覆盖之前的分组设置
...
//   - 字段名最终会使用 SQL Server 的方括号引用: [column] ASC/DESC
//   - 多次调用会覆盖之前的排序设置

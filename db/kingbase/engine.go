...
// ConnectDatabase 连接 Kingbase（人大金仓）数据库
//
// 参数:
...
//
// 返回:
//   - *Engine: Kingbase 数据库操作实例
//
// 说明:
//   - 如果 Engine 不匹配则 panic
//   - 需要手动添加 Kingbase 官方 Go 驱动包，例如 _ "gitee.com/kingbase/kb_go_driver"
...
// Engine 结构体用于管理 Kingbase 数据库连接和操作
type Engine struct {
	name        string   // 数据库连接名称
	DB          *sql.DB  // 数据库连接对象
	...
	limitSQL    string   // LIMIT/OFFSET 分页语句
...
//
// 说明:
//   - 使用 ? 作为统一占位符，最终会被转换为 Kingbase 的 $1,$2,... 格式
//   - 支持在事务中使用
...
//
// 说明:
//   - Kingbase 不区分数据库名，通常通过 schema 管理命名空间
//   - 检查表是否已存在，存在则跳过创建
//   - 解析模型结构体的字段标签
//   - 支持自定义迁移前后处理函数
//   - 创建失败时会panic
func (engine *Engine) Migrate(schema string, model Model) {
	...
	row := engine.QueryRow("SELECT 1 FROM information_schema.tables WHERE table_schema = ? AND table_name = ?;", schema, settings.TableName)
	...
	createSQL := fmt.Sprintf(`CREATE TABLE "%v"."%v" (%v)`, schema, settings.TableName, columnsSQL)
	...
	migrationModelMsg := i18n.T("db.migration.kingbase", map[string]any{
		"engine":  driverName,
		"name":    engine.name,
		"db_name": schema,
...
//   - 使用 ? 作为统一占位符，最终会在 Execute/Query 中被转换为 Kingbase 的 $1,$2,... 格式
...
// 说明:
//   - 字段名会被自动去除首尾空格和引号
//   - 字段名最终会使用 Kingbase 的双引号引用: "column"
//   - 空字段名会被忽略
//   - 多次调用会覆盖之前的分组设置
...
//   - 字段名最终会使用 Kingbase 的双引号引用: "column" ASC/DESC
//   - 多次调用会覆盖之前的排序设置
...
//   - Kingbase 使用 LIMIT pageSize OFFSET offset 实现分页
func (engine *Engine) Page(page int64, pageSize int64) (int64, int64, error) {
	...
	engine.limitSQL = fmt.Sprintf(" LIMIT %d OFFSET %d", pageSize, offset)

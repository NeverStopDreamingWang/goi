...
// 预编译的正则表达式
var (
	identityRe = regexp.MustCompile(`(?i)SERIAL|GENERATED\s+ALWAYS\s+AS\s+IDENTITY|GENERATED\s+BY\s+DEFAULT\s+AS\s+IDENTITY`)
	defaultRe  = regexp.MustCompile(`(?i)DEFAULT`)
	notNullRe  = regexp.MustCompile(`(?i)NOT\s+NULL`)
	varcharRe  = regexp.MustCompile(`(?i)VARCHAR\((\d+)\)`)
)

// PostgreSQL Engine Validate 验证器
// attrs 验证数据
...
func isNotNullValidate(fieldType string) bool {
	// PostgreSQL 中 SERIAL / GENERATED BY DEFAULT AS IDENTITY 等自动生成字段视为可空
	if identityRe.MatchString(fieldType) {
		return false
	}
